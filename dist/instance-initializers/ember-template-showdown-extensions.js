...
function initialize(application) {
  showdown.extension('header-links', function () {
    return [{
      ...
      replace: `$1$4 <a href="#$3"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
  ...
  showdown.extension('feature-flags', function () {
    return [{
      ...
      filter: function (text) {
        return text.replace(/<feature-flag-on-([^>]+)>([\s\S]*?)<\/feature-flag-on-\1>/g, function (match, featureName, content) {
          if (features.isEnabled(featureName)) {
            return content;
          } else {
            return '';
          }
        }).replace(/<feature-flag-off-([^>]+)>([\s\S]*?)<\/feature-flag-off-\1>/g, function (match, featureName, content) {
          if (!features.isEnabled(featureName)) {
            return content;
          } else {
            return '';
...
var emberTemplateShowdownExtensions = {
  initialize
};

export { emberTemplateShowdownExtensions as default, initialize };
//# sourceMappingURL=ember-template-showdown-extensions.js.map
